...
public sealed record New() : TicketStatus()
{
    public override bool CanTransitionTo(TicketStatus newStatus)
        => newStatus is Opened;
}

public sealed record Opened() : TicketStatus()
{
    public override bool CanTransitionTo(TicketStatus newStatus)
        => newStatus is InProgress or Escalated or Closed;
}

public sealed record InProgress() : TicketStatus()
{
    public override bool CanTransitionTo(TicketStatus newStatus)
        => newStatus is WaitingForUser or Resolved or Escalated;
}

public sealed record WaitingForUser() : TicketStatus()
{
    public override bool CanTransitionTo(TicketStatus newStatus)
        => newStatus is InProgress or Resolved;
}

public sealed record Escalated() : TicketStatus()
{
    public override bool CanTransitionTo(TicketStatus newStatus)
        => newStatus is InProgress or Resolved;
}

public sealed record Resolved() : TicketStatus()
{
    public override bool CanTransitionTo(TicketStatus newStatus)
        => newStatus is Closed or Reopened;
}

public sealed record Reopened() : TicketStatus()
{
    public override bool CanTransitionTo(TicketStatus newStatus)
        => newStatus is InProgress or Escalated;
}

public sealed record Closed() : TicketStatus()
{
    public override bool CanTransitionTo(TicketStatus newStatus)
